    ...
    <div className="flex flex-col gap-3">
            ...
            {images.map((img, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Done — show compression savings badge */}
                  {img.status === 'done' && (
                    <>
                      ...
                      {savings !== null && savings > 0 && (
                        <span className="absolute bottom-1 left-1.5 rounded-sm bg-black/60 px-1.5 py-0.5 font-mono text-[8px] text-green-400">
                          −{savings}%
                        </span>
                      )}
                    </>
      ...
      {canAddMore && (
        <div
          ...
          className={cn(
            'flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed px-4 transition-all',
            error
              ? 'border-destructive/50 bg-destructive/5'
              : 'border-border hover:border-accent/50 hover:bg-accent/5',
            images.length === 0 ? 'py-12' : 'py-5'
          )}
        >
          {images.length === 0 ? (
            <>
              ...
              <div className="text-center">
                ...
                <p className="mt-0.5 font-mono text-[9px] text-muted-foreground/40">
                  ✨ Auto-converted to WebP for optimal size
                </p>
              </div>
            </>
          ) : (
      ...
      {/* Compression summary when all done */}
      {images.length > 0 && !isAnyBusy && images.every((i) => i.status === 'done') && (
        (() => {
          const totalOrig = images.reduce((s, i) => s + (i.originalSize ?? 0), 0);
          const totalComp = images.reduce((s, i) => s + (i.compressedSize ?? 0), 0);
          const saved     = totalOrig - totalComp;
          if (saved <= 0) return null;
          return (
            <p className="font-mono text-[10px] text-green-500/80">
              ✓ Saved {formatFileSize(saved)} ({Math.round((saved / totalOrig) * 100)}%) via WebP compression
            </p>
          );
        })()
      )}
