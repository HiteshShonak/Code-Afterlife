...
export async function POST(
  ...
  try {
    ...
    if (!process.env.GROQ_API_KEY) {
      logger.error('Chat API requested without GROQ_API_KEY');
      return NextResponse.json({ message: 'AI chat is not configured' }, { status: 503 });
    }
...
3. STALLED: A project decays into STALLED state automatically if there has been NO activity for exactly ${HEALTH_CONFIG.stalledDays} days.
4. DEAD: A project decays into DEAD state automatically if there has been NO activity for exactly ${HEALTH_CONFIG.deadDays} days. Once DEAD, it can be resurrected by anyone to spawn a new child project (lineage).
...
function getStateDescription(state: string): string {
  const map: Record<string, string> = {
    ...
    STALLED: `no activity for ${HEALTH_CONFIG.stalledDays} days`,
